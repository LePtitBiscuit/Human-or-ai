...
router.get("/debug/routes", (req, res) => {
  res.json({
    ...
    routes: [
      ...
      "GET /debug/routes - Liste des routes API",
      "POST /auth - Authentification avec code PIN",
      "GET /auth/verify - Vérification du token JWT",
      "GET /auth/info - Informations sur l'authentification",
      "GET /getgames - Récupérer toutes les parties",
      "POST /creategame - Créer une nouvelle partie",
      "POST /launchgame/:id - Lancer une partie",
      "POST /stopgame/:id - Arrêter une partie",
      "DELETE /devices/:gameId/remove/:deviceSocketId - Supprimer un appareil d'une partie",
      "GET /devices/default - Récupérer la liste des devices par défaut",
      "GET /devices/available - Récupérer la liste des devices par défaut disponibles",
      "GET /devices/pending-requests - Récupérer la liste des demandes d'autorisation en attente",
    ],
    timestamp: new Date().toISOString(),
  });
});
